        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <Label className="text-sm font-semibold text-foreground">QR Type</Label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    role="combobox"
                    className="h-12 w-full justify-between rounded-2xl border-border bg-muted/70 px-4 text-foreground"
                  >
                    {formData.type ? formatQrType(formData.type) : 'Select'}
                    <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[220px] rounded-2xl border border-border bg-card p-1.5 shadow-xl">
                  <Command className="rounded-2xl bg-card text-foreground [&_[cmdk-input-wrapper]]:border-b-0 [&_[cmdk-input-wrapper]]:px-0">
                      ...
                      <CommandGroup className="space-y-1 p-1.5">
                        {(['AREA', 'CLOCK_IN', 'CLOCK_OUT', 'TASK', 'FEEDBACK', 'RAW'] as ManualQRCodePayload['type'][]).map(type => (
                          <CommandItem
                            key={type}
                            value={type}
                            className="rounded-xl px-3 py-2 text-sm text-foreground data-[selected=true]:bg-primary/10 data-[selected=true]:text-primary"
                            onSelect={() => {
            ...
            <div className="space-y-2">
              ...
              <Popover>
                ...
                <PopoverContent className="w-[260px] rounded-2xl border border-border bg-card p-1.5 shadow-xl">
                  <Command className="rounded-2xl bg-card text-foreground [&_[cmdk-input-wrapper]]:border-b-0 [&_[cmdk-input-wrapper]]:px-0">
                    <CommandInput
                      placeholder="Search customers"
                      value={customerSearch}
                      onValueChange={setCustomerSearch}
                      className="h-10 rounded-xl border border-border bg-muted px-3 text-sm text-foreground placeholder:text-muted-foreground"
                    />
                    ...
                    <CommandList className="max-h-56">
                      <CommandGroup className="space-y-1 p-1.5">
                        {filteredCustomers.length === 0 && !isLoadingCustomers ? (
                          <CommandItem disabled value="no-results" className="rounded-xl px-3 py-2 text-sm text-muted-foreground">
                            No customers yet
                          </CommandItem>
                        ) : null}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-12 rounded-2xl border-border bg-muted/70 px-4 text-foreground"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <Button
              ...
              className="flex-1 rounded-full bg-primary py-5 text-lg font-semibold text-primary-foreground shadow-lg transition hover:bg-primary/90"
            >
              {isGenerating ? (
        ...
        <Card className="border-0 shadow-xl rounded-3xl">
          <CardHeader>
            <CardTitle className="flex items-center gap-3 text-xl font-semibold text-foreground">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-success/10">
            ...
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-border bg-muted/70 p-4">
                  ...
                  <p className="mt-1 text-base font-semibold text-foreground">{result.qrData.customerName}</p>
                </div>
                {result.qrData.metadata?.areaName && (
                  <div className="rounded-2xl border border-border bg-muted/70 p-4">
                    <Label className="text-xs uppercase text-muted-foreground">Name</Label>
                    <p className="mt-1 text-base font-semibold text-foreground">{result.qrData.metadata.areaName}</p>
                  </div>
                )}
              </div>

              <div className="flex flex-col gap-3 sm:flex-row">
                <Button onClick={downloadQR} className="flex-1 rounded-full bg-primary py-4 text-primary-foreground">
                  <Download className="mr-2 h-4 w-4" />
                  Download PNG
                </Button>
                <Button onClick={copyPayload} variant="outline" className="flex-1 rounded-full border-border py-4 text-foreground">
                  {copied ? (
